...
/**
 * Custom Cache impl slight modification on Jake Worthon's disk cache
 * 
 * @author boyan
 *
 */
public class DiskLruImageCache implements ImageCache {
	...
	private static final String CACHE_TEST_DISK = DiskLruImageCache.class.getCanonicalName();
		...
		try {
			...
			if (writeBitmapToFile(data, editor)) {
				...
				if (BuildConfig.DEBUG) {
					Log.d(CACHE_TEST_DISK, "image put on disk cache " + key);
				}
			} else {
				editor.abort();
				if (BuildConfig.DEBUG) {
					Log.d(CACHE_TEST_DISK,
							"ERROR on: image put on disk cache " + key);
				}
			}
		} catch (IOException e) {
			if (BuildConfig.DEBUG) {
				Log.d(CACHE_TEST_DISK, "ERROR on: image put on disk cache "
						+ key);
			}
			try {
				if (editor != null) {
	...
	public Bitmap getBitmap(String key) {
		...
		// we must match [a-z0-9_-]{1,64} and we're getting a url so... haxfix
		// time
		key = sanitizeKey(key);
		...
		if (BuildConfig.DEBUG) {
			Log.d(CACHE_TEST_DISK, bitmap == null ? ""
